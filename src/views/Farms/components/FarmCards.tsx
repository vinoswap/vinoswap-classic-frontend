...
const FarmCards: React.FC = () => {
  ...
  const grapeIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'GRAPE',
  )

  const grapePrice =
    grapeIndex >= 0 && stakedValue[grapeIndex]
      ? stakedValue[grapeIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const GRAPE_PER_BLOCK = new BigNumber(50)
    ...
    <StyledCards>
      {!!rows[0].length ? (
        ...
        <StyledLoadingWrapper>
          <Loader text="Decanting the wine ..." />
        </StyledLoadingWrapper>
      )}
    </StyledCards>
  )
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [grape, lpTokenAddress, account, setHarvestable])
